...
use crate::execute::{ProcIO, LashWait, RustFd};
...
use crate::shellenv::{self, disable_reaping, enable_reaping, read_jobs, read_logic, read_meta, read_vars, write_jobs, write_logic, write_meta, write_vars, ChildProc, EnvFlags, HashFloat, JobBuilder, JobCmdFlags, JobID, LashVal};
...
fn open_file_descriptors(redirs: VecDeque<Node>) -> LashResult<Vec<RustFd>> {
...
pub fn r#type(node: Node) -> LashResult<LashWait> {
	...
	while let Some(arg) = argv.pop_front() {
		...
		if let Some(caps) = index_re.captures(text) {
			...
			if let Some(LashVal::Array(_)) = read_vars(|v| v.get_var(var_name))? {
				if let Ok(arr_element) = read_vars(|v| v.index_arr(var_name, index))? {
					use crate::shellenv::LashVal::*;
					match arr_element {
...
pub fn read_func(node: Node, io: ProcIO) -> LashResult<LashWait> {
	let span = node.span();
...
pub fn shift(node: Node) -> LashResult<LashWait> {
	let span = node.span();
...
pub fn string(node: Node) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
	argv.pop_front(); // Ignore `string`
	while let Some(arg) = argv.pop_front() {
		if let Some((key,val)) = arg.text().split_once('=') {
			helper::unset_var_conflicts(key)?;
			write_vars(|v| v.set_var(key, LashVal::String(val.to_string())))?
		} else {
...
pub fn int(node: Node) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
	argv.pop_front(); // Ignore `int`
	while let Some(arg) = argv.pop_front() {
		if let Some((k,v)) = arg.text().split_once('=') {
			...
			if let Ok(int) = v.parse::<i32>() {
				write_vars(|v| v.set_var(k, LashVal::Int(int)))?
			} else {
...
pub fn bool(node: Node) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
	argv.pop_front(); // Ignore `int`
	while let Some(arg) = argv.pop_front() {
		if let Some((k,val)) = arg.text().split_once('=') {
			...
			if let Ok(bool) = val.parse::<bool>() {
				write_vars(|v| v.set_var(k, LashVal::Bool(bool)))?
			}
		} else {
			return Err(ShError::from_syntax(format!("Expected assignment syntax like `key=value`, got `{}`",arg.text()).as_str(), node.span()))
		}
...
pub fn float(node: Node) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
	argv.pop_front(); // Ignore `int`
	while let Some(arg) = argv.pop_front() {
		if let Some((k,val)) = arg.text().split_once('=') {
			...
			if let Ok(float) = val.parse::<f64>() {
				write_vars(|v| v.set_var(k, LashVal::Float(HashFloat(float))))?
			}
		} else {
			return Err(ShError::from_syntax(format!("Expected assignment syntax like `key=value`, got `{}`",arg.text()).as_str(), node.span()))
		}
...
pub fn array(node: Node) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
	argv.pop_front(); // Ignore `int`
	while let Some(arg) = argv.pop_front() {
		if let Some((k,val)) = arg.text().split_once('=') {
			...
			if let Ok(array) = helper::parse_vec(val) {
				write_vars(|v| v.set_var(k, LashVal::Array(array)))?
			}
		} else {
			return Err(ShError::from_syntax(format!("Expected assignment syntax like `key=value`, got `{}`",arg.text()).as_str(), node.span()))
		}
...
/// let transform = |s: String| -> LashResult<i32> { Ok(s.parse::<i32>().unwrap()) };
...
fn do_logical_op(
	...
	let result2 = test(std::mem::take(args)).map(|res| matches!(res,LashWait::Success ))?;
	match operator.text() {
		"!" => { Ok(!result2) },
...
/// # Returns
///
/// * `Ok(LashWait)` - Returns a success or failure status based on the evaluation of the conditional expression.
...
pub fn test(mut argv: VecDeque<Tk>) -> LashResult<LashWait> {
	let span = Span::from(argv.front().unwrap().span().start,argv.back().unwrap().span().end);
	let is_false = || -> LashResult<LashWait> { Ok(LashWait::Fail { code: 1, cmd: Some("test".into()), }) };
	let is_true = || -> LashResult<LashWait> { Ok(LashWait::Success ) };
	let is_int = |tk: &Tk| -> bool { tk.text().parse::<i32>().is_ok() };
	let to_int = |tk: Tk| -> LashResult<i32> {
...
pub fn expr(node: Node, io: ProcIO) -> LashResult<LashWait> {
...
pub fn setopt(node: Node) -> LashResult<LashWait> {
	let mut argv: VecDeque<Tk> = node.get_argv()?.into();
	argv.pop_front();
	while let Some(arg) = argv.pop_front() {
...
pub fn getopt(node: Node, io: ProcIO) -> LashResult<LashWait> {
	let mut argv: VecDeque<Tk> = node.get_argv()?.into();
	argv.pop_front();
	while let Some(arg) = argv.pop_front() {
...
pub fn alias(node: Node) -> LashResult<LashWait> {
	let mut argv: VecDeque<Tk> = node.get_argv()?.into();
	argv.pop_front();
	while let Some(arg) = argv.pop_front() {
...
pub fn unalias(node: Node) -> LashResult<LashWait> {
	let mut argv: VecDeque<Tk> = node.get_argv()?.into();
	argv.pop_front();
	while let Some(arg) = argv.pop_front() {
		let alias = arg.text();
...
pub fn cd(node: Node, flags: CdFlags) -> LashResult<LashWait> {
	let mut argv = VecDeque::from(node.get_argv()?);
		argv.pop_front();
		let new_pwd;
...
pub fn cd_internal(path: String, span: Span, flags: CdFlags) -> LashResult<LashWait> {
	let mut tokens = vec![
		Tk {
...
pub fn fg(node: Node) -> LashResult<LashWait> {
...
pub fn bg(node: Node) -> LashResult<LashWait> {
	let argv = node.get_argv()?;
	let mut argv = VecDeque::from(argv);
...
pub fn pushd(node: Node) -> LashResult<LashWait> {
	...
	node_operation!(NdType::Builtin { mut argv }, node, {
		...
		if let Some(path) = args.last() {
			...
			Ok(LashWait::Fail { code: 1, cmd: Some("pushd".into()) })
		}
	})
}

pub fn popd(node: Node) -> LashResult<LashWait> {
	...
	if let Some(path) = result {
		if let Some(string) = path.to_str() {
			...
			Ok(LashWait::Fail { code: 1, cmd: Some("popd".into()) })
		}
	} else {
		Ok(LashWait::Fail { code: 1, cmd: Some("popd".into()) })
	}
...
fn parse_job_id(arg: &str, span: Span) -> LashResult<usize> {
	if arg.starts_with('%') {
...
pub fn source(node: Node) -> LashResult<LashWait> {
	let mut argv = node
		.get_argv()?
		.iter()
...
pub fn set_or_unset(node: Node, set: bool) -> LashResult<LashWait> {
	let span = node.span();
...
pub fn pwd(span: Span) -> LashResult<LashWait> {
...
pub fn export(node: Node) -> LashResult<LashWait> {
	let last_status = LashWait::Success ;
	if let NdType::Builtin { mut argv } = node.nd_type {
...
pub fn jobs(node: Node, mut io: ProcIO,) -> LashResult<LashWait> {
...
pub fn echo_internal(argv: Vec<String>, io: ProcIO, span: Span, flags: NdFlags, redirs: VecDeque<Node>) -> LashResult<LashWait> {
	let mut tokens = vec![
		Tk {
			tk_type: TkType::Ident,
			wd: token::WordDesc {
...
pub fn echo(node: Node, mut io: ProcIO,) -> LashResult<LashWait> {
