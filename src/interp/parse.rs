...
mod tests {
	...
	#[test]
	fn test_descend_basic() {
			let input = "echo hello";
			let shellenv = ShellEnv::new(false,true); // Replace with appropriate setup if needed

			let result = descend(input, &shellenv);
			assert!(result.is_ok(), "Parsing failed for basic input");

			let state = result.unwrap();
			// Check that the tokens match the expected result
			assert_eq!(state.tokens.len(), 4, "Unexpected number of tokens");
			assert_eq!(state.tokens[1].text(), "echo", "First token mismatch");
			assert_eq!(state.tokens[2].text(), "hello", "Second token mismatch");

			// Validate the AST structure
			if let NdType::Root { deck } = state.ast.nd_type {
					assert_eq!(deck.len(), 1, "Unexpected number of nodes in AST");
			} else {
					panic!("Expected AST root node to have type Root");
			}
	}

	#[test]
	fn test_descend_if_statement() {
			let input = "if true; then echo hello; fi";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_ok(), "Parsing failed for `if` statement");

			let state = result.unwrap();
			// Validate the AST structure for `if`
			if let NdType::Root { deck } = state.ast.nd_type {
					assert_eq!(deck.len(), 1, "Unexpected number of nodes in AST");
					if let NdType::If { cond_blocks, else_block } = &deck.front().unwrap().nd_type {
							assert_eq!(cond_blocks.len(), 1, "Unexpected number of conditional blocks");
							assert!(else_block.is_none(), "Else block should be None for this `if`");
					} else {
							panic!("Expected `if` node in AST");
					}
			}
	}
	#[test]
	fn test_descend_pipeline() {
			let input = "echo hello | grep hello";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_ok(), "Parsing failed for pipeline input");

			let state = result.unwrap();
			// Validate the AST structure for pipeline
			if let NdType::Root { deck } = state.ast.nd_type {
					assert_eq!(deck.len(), 1, "Unexpected number of nodes in AST");
					if let NdType::Pipeline { left, right, .. } = &deck.front().unwrap().nd_type {
							// Validate left and right nodes
							assert!(matches!(left.nd_type,NdType::Builtin { .. }), "Left node is not a command");
							assert!(matches!(right.nd_type, NdType::Command { .. }), "Right node is not a command");
					} else {
							panic!("Expected `Pipeline` node in AST");
					}
			}
	}

	#[test]
	fn test_descend_if_missing_fi() {
			let input = "if true; then echo hello";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `fi`");

			let error = result.unwrap_err();
			assert!(matches!(error, ParseErr {..}), "Unexpected error type");
	}
	#[test]
	fn test_descend_if_missing_then() {
			let input = "if true; echo hello; fi";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `then`");

			let error = result.unwrap_err();
			assert!(matches!(error, ParseErr {..}), "Unexpected error type");
	}
	#[test]
	fn test_descend_else_before_elif() {
			let input = "if true; else echo; elif true; then echo; fi";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for else before elif");

			let error = result.unwrap_err();
			assert!(matches!(error, ParseErr {..}), "Unexpected error type");
	}
	#[test]
	fn test_descend_if_no_body() {
			let input = "if true; fi";
			let shellenv = ShellEnv::new(false,true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for if statement with no body");
	...
	#[test]
	fn test_descend_while_missing_done() {
			let input = "while true; do echo hello";
			let shellenv = ShellEnv::new(false, true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `done`");
	...
	#[test]
	fn test_descend_while_missing_do() {
			let input = "while true; echo hello; done";
			let shellenv = ShellEnv::new(false, true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `do`");
	...
	#[test]
	fn test_descend_until_missing_done() {
	...
	#[test]
	fn test_descend_until_missing_do() {
			let input = "until true; echo hello; done";
			let shellenv = ShellEnv::new(false, true);
	...
	#[test]
	fn test_descend_for_basic() {
			let input = "for i in 1 2 3; do echo $i; done";
			let shellenv = ShellEnv::new(false, true);

			let result = descend(input, &shellenv);
			assert!(result.is_ok(), "Parsing failed for basic `for` loop");
			...
			if let NdType::Root { deck } = state.ast.nd_type {
					...
					if let NdType::For { loop_vars, loop_arr, loop_body } = &deck.front().unwrap().nd_type {
							...
							panic!("Expected `For` node in AST");
					}
			}
	}

	#[test]
	fn test_descend_for_missing_done() {
	...
	#[test]
	fn test_descend_for_missing_in() {
			let input = "for i 1 2 3; do echo $i; done";
			let shellenv = ShellEnv::new(false, true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `in`");
	...
	#[test]
	fn test_descend_for_no_loop_vars() {
			let input = "for in 1 2 3; do echo $i; done";
			let shellenv = ShellEnv::new(false, true);

			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing loop variable");
	...
	fn test_descend_case_basic() {
			...
			let result = descend(input, &shellenv);
			assert!(result.is_ok(), "Parsing failed for basic `case` statement");
			...
			if let NdType::Root { deck } = state.ast.nd_type {
					...
					if let NdType::Case { input_var, cases } = &deck.front().unwrap().nd_type {
							...
							panic!("Expected `Case` node in AST");
					}
			}
	}

	#[test]
	fn test_descend_case_missing_esac() {
			let input = r#"
					case $var in
							option1)
									echo "Option 1"
			...
			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing `esac`");
	...
	#[test]
	fn test_descend_case_missing_in() {
			let input = r#"
					case $var
							option1)
									echo "Option 1"
	...
	#[test]
	fn test_descend_case_missing_pattern() {
			let input = r#"
					case $var in
							)
									echo "No pattern"
			...
			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for missing case pattern");
	...
	#[test]
	fn test_descend_case_no_branches() {
			let input = r#"
					case $var in
					esac
			"#;
			...
			let result = descend(input, &shellenv);
			assert!(result.is_err(), "Expected parsing error for `case` statement with no branches");
